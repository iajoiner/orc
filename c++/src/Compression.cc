...
CompressionStreamBase::CompressionStreamBase(OutputStream* outStream,
                                             int compressionLevel, uint64_t capacity,
                                             uint64_t blockSize, MemoryPool& pool)
...
ZlibCompressionStream::ZlibCompressionStream(OutputStream* outStream,
                                             int compressionLevel, uint64_t capacity,
                                             uint64_t blockSize, MemoryPool& pool)
...
class ZlibDecompressionStream : public DecompressionStream {
 ...
 protected:
  virtual void NextDecompress(const void** data, int* size,
...
ZlibDecompressionStream::ZlibDecompressionStream(
    std::unique_ptr<SeekableInputStream> inStream, size_t bufferSize, MemoryPool& _pool)
...
void ZlibDecompressionStream::NextDecompress(const void** data, int* size,
  ...
  do {
    result =
        inflate(&zstream, availableSize == remainingLength ? Z_FINISH : Z_SYNC_FLUSH);
    switch (result) {
      case Z_OK:
        remainingLength -= availableSize;
        inputBuffer += availableSize;
...
BlockDecompressionStream::BlockDecompressionStream(
    std::unique_ptr<SeekableInputStream> inStream, size_t blockSize, MemoryPool& _pool)
    : DecompressionStream(std::move(inStream), blockSize, _pool),
...
void BlockDecompressionStream::NextDecompress(const void** data, int* size,
  ...
  if (remainingLength == availableSize) {
    ...
    for (size_t pos = availableSize; pos < remainingLength;) {
      readBuffer(true);
      size_t avail = std::min(static_cast<size_t>(inputBufferEnd - inputBuffer),
                              remainingLength - pos);
...
uint64_t SnappyDecompressionStream::decompress(const char* _input, uint64_t length,
...
uint64_t LzoDecompressionStream::decompress(const char* inputPtr, uint64_t length,
                                            char* output, size_t maxOutputLength) {
...
uint64_t Lz4DecompressionStream::decompress(const char* inputPtr, uint64_t length,
                                            char* output, size_t maxOutputLength) {
...
class SnappyCompressionStream : public CompressionStream {
 public:
  SnappyCompressionStream(OutputStream* outStream, int compressionLevel,
...
SnappyCompressionStream::SnappyCompressionStream(OutputStream* outStream,
                                                 int compressionLevel, uint64_t capacity,
                                                 uint64_t blockSize, MemoryPool& pool)
    : CompressionStream(outStream, compressionLevel, capacity, blockSize, pool) {
  init();
...
uint64_t SnappyCompressionStream::doStreamingCompression() {
  ...
  snappy::RawCompress(static_cast<const char*>(data), size, outputBuffer,
                      &compressedSize);
  outputPosition += static_cast<int>(compressedSize);
  return compressedSize;
}

std::string SnappyCompressionStream::getName() const { return "SnappyCompressionStream"; }
...
uint64_t ZSTDDecompressionStream::decompress(const char* inputPtr, uint64_t length,
                                             char* output, size_t maxOutputLength) {
...
std::unique_ptr<BufferedOutputStream> createCompressor(
    ...
    uint64_t bufferCapacity, uint64_t compressionBlockSize, MemoryPool& pool) {
  switch (static_cast<int64_t>(kind)) {
    ...
    case CompressionKind_ZLIB: {
      int level = (strategy == CompressionStrategy_SPEED) ? Z_BEST_SPEED + 1
